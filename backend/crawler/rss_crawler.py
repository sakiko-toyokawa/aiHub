...
class RssCrawler(BaseCrawler):
    ...
    async def fetch(self, **kwargs) -> List[CrawlResult]:
        ...
        logger.info(f"[RSS] Fetching feed: {self.feed_url} (expanded={expanded}, max={max_items})")
        ...
        items = self._parse_feed(xml_text, max_items)
    ...
    def _parse_feed(self, xml_text: str, max_items: int) -> List[CrawlResult]:
        ...
        if root_tag == "rss":
            ...
            if channel is not None:
                ...
                for item in entries:
                    results.append(self._parse_rss_item(item, feed_title, ns))
        elif root_tag == "feed":
            # Atom
            feed_title = self._text(root.find("atom:title", ns))
            entries = root.findall("atom:entry", ns)[:max_items]
            for entry in entries:
                results.append(self._parse_atom_entry(entry, feed_title, ns))
        else:
