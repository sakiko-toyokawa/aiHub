...
def init_sample_data():
    ...
    try:
        ...
        # 查询已创建的 sources，建立 platform -> source_id 映射
        sources_map = {s.platform: s.id for s in db.query(Source).all()}
        ...
        for data in raw_content_data:
            source_id = sources_map.get(data["platform"])
            rc = RawContent(
                source_id=source_id,
        ...
        for data in summary_data:
            raw_id = raw_contents[data["raw_content_idx"]].id
            s = Summary(
        ...
        user_read_data = [
            {"summary_idx": 1, "is_read": True, "is_favorited": True},
            {"summary_idx": 3, "is_read": True, "is_favorited": False},
        ]

        user_reads = []
        for data in user_read_data:
            summary_id = summaries[data["summary_idx"]].id
            ur = UserRead(
